...
def extract_trajectory(
    ...
    Args:
        env (instance of EnvBasen): environment
...
def dataset_states_to_obs(args):
    ...
    for ind in range(len(demos)):
        ...
        states = f["data/{}/states".format(ep)][()] # gets the states from the hdf5 file for this demo
        initial_state = dict(states=states[0])
        if is_robosuite_env:
            initial_state["model"] = f["data/{}".format(ep)].attrs["model_file"]

        # extract obs, rewards, dones
        actions = f["data/{}/actions".format(ep)][()] # gets the actions from the hdf5 file for this demo
