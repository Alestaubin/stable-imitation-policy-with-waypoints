...
######## Data Preprocessing ########
python lib/utils/preprocess_hdf5.py -i "${DATA_PATH}/${SCENARIO_NAME}.hdf5" -o "$MODIFIED_DATA_PATH"
...
# NOTE: THIS IS NOT OPTIONAL, must playback sim to exctract the euler angles
python sim/playback_robomimic_dataset.py --dataset "$DATA_PATH/$OUTPUT_NAME" --video_path "$DATA_PATH/demo.mp4" --use_actions --absolute

########## AWE waypoints ##########
# convert delta actions to absolute actions
python lib/utils/robomimic_convert_action.py --dataset "$DATA_PATH/$OUTPUT_NAME" --start_idx 1 --end_idx 1

# extract waypoints
python lib/utils/waypoint_extraction.py --dataset "$DATA_PATH/$OUTPUT_NAME" --group_name "AWE_waypoints_0025" --start_idx 1 --end_idx 1 --err_threshold 0.0025
