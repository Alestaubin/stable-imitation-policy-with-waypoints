...
######## Data Preprocessing ########
#python lib/utils/preprocess_hdf5.py -i "${DATA_PATH}/${SCENARIO_NAME}.hdf5" -o "$MODIFIED_DATA_PATH" --libero_folder "libero_goal" #--bddl_file "stable-imitation-policy-with-waypoints/stable-imitation-policy-with-waypoints/${DATA_PATH2}/${SCENARIO_NAME}.bddl"
...
#python sim/playback_robomimic_dataset.py --dataset "$DATA_PATH/$OUTPUT_NAME" --video_path "$DATA_PATH/demo.mp4" --start_idx 0 --end_idx 30
#printf "Playback video saved to $DATA_PATH/demo.mp4\n"

########## AWE waypoints ##########
# convert delta actions to absolute actions
#python lib/utils/robomimic_convert_action.py --dataset "$DATA_PATH/$OUTPUT_NAME" --start_idx 0 --end_idx 30
